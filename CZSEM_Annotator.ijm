...
macro "Add Multiple Lines of SEM Metadata to Image" {
	...
	textChoiceLines = favoriteParameters.length; /* Number of optional lines */
	if (metaTagArray.length<(213+textChoiceLines)) restoreExit("Header appears to be non-standard"); 
	for (i = 0; i<favoriteParameters.length; i++) {
		for (j = 0; j<metaTagArray.length; j++) {
...
*/
	...
	function ExtractZeissSEMParametersFromMetadata(metaArray) {
		parameterTempArray = newArray(metaArray.length);
		for (i=0; i<metaArray.length; i++) {
	...
	function ExtractZeissSEMValuesFromMetadata(metaArray) {
		valueTempArray = newArray(metaArray.length);
		for (i=0; i<metaArray.length; i++) {
	...
	function ExtractZeissSEMUnitsFromMetadata(metaArray) {
		...
		for (i=0; i<metaArray.length; i++) {
			...
			if (splits.length<=1) unitsTempArray[i] = "";
			else if (splits.length==2) unitsTempArray[i] = splits[1];
